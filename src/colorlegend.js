...
exports.colorlegend = function(target, scale, type, options) {
	var scaleTypes = [ 'linear', 'quantile', 'ordinal' ], found = false, opts = options
	...
	, linearBoxes = opts.linearBoxes || 9 // number of boxes for linear scales
	// (int)
	, htmlElement = document
			.getElementById(target.substring(0, 1) === '#' ? target.substring(
					1, target.length) : target) // target container element -
	// strip the prefix #
	, w = htmlElement.offsetWidth // width of container element
	, h = htmlElement.offsetHeight // height of container element
	, colors = [], padding = [ 2, 4, 10, 4 ] // top, right, bottom, left
	, boxSpacing = type === 'ordinal' ? 3 : 0 // spacing between boxes
	, titlePadding = title ? 11 : 0, domain = scale.domain(), range = scale
			.range(), i = 0, isVertical = opts.vertical || false;
	...
	if (!isVertical) {
		if (fill
				|| w < (boxWidth + boxSpacing) * colors.length + padding[1]
						+ padding[3]) {
			boxWidth = (w - padding[1] - padding[3] - (boxSpacing * colors.length))
					/ colors.length;
		}
		...
		if (fill
				|| h < (boxHeight + boxSpacing) * colors.length + padding[0]
						+ padding[2]) {
			boxHeight = (h - padding[0] - padding[2] - (boxSpacing * colors.length))
					/ colors.length;
		}
	...
	var legend = d3.select(target).append('svg').attr('width', w).attr(
			'height', h).append('g').attr('class', 'colorlegend').attr(
			'transform', 'translate(' + padding[3] + ',' + padding[0] + ')')
			.style('font-size', '11px').style('fill', '#666');

	var legendBoxes = legend.selectAll('g.legend').data(colors).enter().append(
			'g');

	// value labels
	var valueLabels;
	if (!isVertical) {
		valueLabels = legendBoxes.append('text').attr('class',
				'colorlegend-labels').attr('dy', '.71em').attr(
				'x',
				function(d, i) {
		...
		valueLabels = legendBoxes.append('text').attr('class',
				'colorlegend-labels').attr('dy', padding[0]).attr('x',
				function() {
	...
	valueLabels.style('text-anchor', function() {
	...
	if (!isVertical) {
		legendBoxes.append('rect').attr('x', function(d, i) {
			return i * (boxWidth + boxSpacing);
		}).attr('width', boxWidth).attr('height', boxHeight).style('fill',
				function(d, i) {
					return colors[i];
				});

	} else {
		legendBoxes.append('rect').attr('y', function(d, i) {
	...
	if (title) {
		var legendText = legend.append('text').attr('class',
				'colorlegend-title').style('text-anchor', 'middle').style(
				'pointer-events', 'none').text(title);

		if (!isVertical) {
			legendText.attr('dy', '.71em').attr('x',
					(colors.length * (boxWidth / 2))).attr('y',
					boxHeight + titlePadding);

		} else {
			legendText.attr('dy', '.51em').attr('y',
					(colors.length * (boxHeight / 2))).attr('transform',
					'rotate(90, 5,' + (colors.length * (boxHeight / 2)) + ')');
		}
	}
